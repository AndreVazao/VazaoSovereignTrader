from PC_ENGINE.core.execution_fabric import ExecutionFabric, ExecutionMethod, ExecutionResult, ExecutionTarget
...
def test_android_is_a_transport_not_a_credential_store():
    fabric = ExecutionFabric(
        owner_id="andre",
        adapters={ExecutionMethod.ANDROID: StubAdapter(ExecutionMethod.ANDROID)},
    )
    target = ExecutionTarget(
        ...
        methods=(ExecutionMethod.ANDROID,),
        android_package="example.finance",
    )
    intent = fabric.build_intent(
    ...
    assert fabric.execute(intent).success
