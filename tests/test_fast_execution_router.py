...
def test_router_executes_only_after_risk_check():
    ...
    def executor(decision, payload):
        calls.append((decision.symbol, payload["exchange"]))
        return True

    router = FastExecutionRouter(FastPathEngine(), executor=executor)
    result = router.handle(
        event(),
        (signal(),),
        now_ms=1050,
        risk_check=lambda learned, payload: True,
    )
    assert result.accepted is True
    assert result.executed is True
    assert calls == [("BTC/USDT", "bingx")]


def test_router_cannot_bypass_risk_rejection():
    calls = []

    router = FastExecutionRouter(
        FastPathEngine(),
        executor=lambda decision, payload: calls.append(True) or True,
    )
    result = router.handle(
        event(),
        (signal(),),
        now_ms=1050,
        risk_check=lambda learned, payload: False,
    )
...
def test_router_rejects_stale_event_without_executor_call():
    calls = []
    router = FastExecutionRouter(
        FastPathEngine(max_signal_age_ms=100),
        executor=lambda decision, payload: calls.append(True) or True,
    )
