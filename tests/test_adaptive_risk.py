...
def test_adaptive_risk_scales_only_after_validated_edge():
    ...
    result = controller.evaluate(samples=400, wins=320, mean_net_bps=6, drawdown_pct=0.005)
...
def test_drawdown_blocks_risk_increase():
    ...
    result = controller.evaluate(samples=400, wins=320, mean_net_bps=6, drawdown_pct=0.02)
    assert result.eligible is False
    assert result.multiplier == 0.5
