...
class Adapter:
    ...
    def __init__(self, ok=True, verified=True):
        self.ok, self.verified, self.calls = ok, verified, 0
...
def _setup(tmp_path, enabled=True):
    ...
    adapter = Adapter()
    bridge = CapitalTransferExecutionBridge(owner_id="andre", execution_fabric=ExecutionFabric(owner_id="andre"), intent_store=intents, state_store=states, adapters={ExecutionMethod.API: adapter}, enabled=enabled)
    return bridge, TransferExecutionRequest(intent, "binance-spot", ExecutionMethod.API), adapter, states


def test_disabled_transfer_fails_closed_without_mutating_state(tmp_path):
    bridge, request, adapter, states = _setup(tmp_path, enabled=False)
...
def test_submit_then_reconcile(tmp_path):
    bridge, request, adapter, _ = _setup(tmp_path, enabled=True)
...
def test_without_real_authorization_does_not_submit_or_mutate(tmp_path):
    bridge, request, adapter, states = _setup(tmp_path, enabled=True)
...
def test_owner_mismatch_never_writes_other_owner_state(tmp_path):
    ...
    request = TransferExecutionRequest(
        ...
        account_id=request.account_id, method=request.method,
    )
    try:
        bridge.submit(request=request, real_authorized=True)
