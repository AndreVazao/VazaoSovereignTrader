...
try{
 ...
 $('bridge').innerHTML=items.map(x=>{
   const fs=(x.fields||[]).map(f=>'<div class="muted small">'+(f.label||f.name)+'</div><input data-human="'+x.request_id+'" data-field="'+f.name+'" type="'+(f.type==='secret'?'password':'text')+'" placeholder="'+(f.label||f.name)+'">').join('');
   return '<div class="notice"><b>'+x.title+'</b><div class="muted">'+x.platform+' · '+x.kind+'</div><div>'+x.message+'</div>'+fs+'<div class="row"><button class="go" onclick="respond(\''+x.request_id+'\')">ENVIAR</button><button class="stop" onclick="cancel(\''+x.request_id+'\')">CANCELAR</button></div></div>';
 }).join('');
...
async function refreshResearch(){
 try{const d=await api('/research');$('researchStatus').innerHTML='<span class="ok">'+d.pending+' pendente(s)</span> · '+d.active+' em análise · '+d.completed+' concluída(s) · '+d.discarded+' descartada(s)';$('researchList').innerHTML=(d.requests||[]).slice(0,8).map(x=>'<div class="notice"><b>'+x.status+'</b> · '+new Date(x.created_at*1000).toLocaleString()+'<div>'+x.message+'</div></div>').join('')}catch(e){$('researchStatus').textContent='Pesquisa indisponível'}
}
