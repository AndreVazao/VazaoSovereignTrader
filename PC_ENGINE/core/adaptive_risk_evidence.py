...
class AdaptiveRiskEvidenceStore:
    """Loads bounded PAPER evidence and exposes only fresh, quality-aware statistics."""

    def __init__(self, path: str, *, max_rows: int = 50000, max_age_ms: int = 86_400_000) -> None:
        self.path = Path(path)
    ...
    def lookup(self, *, symbol: str, regime: str | None, horizon_seconds: int | None, action: str = "BUY") -> ContextEvidence | None:
        ...
        # A stable regime requires enough recent observations and confidence.
        # When historical rows are available, compare the latest sample to the
        # preceding aggregate; otherwise use the aggregate confidence itself.
        ...
        return ContextEvidence(samples, wins, mean, lower_ci, eligible, age_ms, stability, source=self._source())
