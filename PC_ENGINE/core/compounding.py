...
class CompoundingController:
    ...
    def __init__(self, *, owner_id: str, venue: str, reserve_cash_pct: float = 0.20, max_equity_multiplier: float = 10_000.0):
        ...
        if not self.owner_id or not self.venue:
            raise ValueError("owner_id and venue are required")
    ...
    def snapshot(self, *, baseline_capital: float, equity: float, adaptive_risk_multiplier: float = 1.0) -> CompoundingSnapshot:
        ...
        return CompoundingSnapshot(
            owner_id=self.owner_id, venue=self.venue, baseline_capital=round(baseline, 8), equity=round(current, 8),
            realized_profit=round(realized_profit, 8), return_pct=round(return_pct, 8),
            reinvestment_equity=round(current, 8), risk_multiplier=round(multiplier, 8),
            transferable_surplus=round(max(0.0, current - baseline), 8),
    ...
    def tier_for_equity(self, *, total_equity: float, seed_capital: float = 1.0, growth_factor: float = 10.0) -> float:
        equity, tier, factor = max(0.0, float(total_equity)), float(seed_capital), float(growth_factor)
    ...
    def next_global_base(self, *, total_equity: float, current_base: float, growth_factor: float = 10.0) -> float:
        equity, base, factor = max(0.0, float(total_equity)), float(current_base), float(growth_factor)
...
class GlobalCompoundingOrchestrator:
    """Coordinate owner-private global 10x capitalization without executing transfers."""
    ...
    def __init__(self, *, owner_id: str, venues: list[str], seed_base: float = 1.0, growth_factor: float = 10.0):
        ...
        self.seed_base, self.growth_factor = float(seed_base), float(growth_factor)
    ...
    def snapshot(self, equities: dict[str, float]) -> GlobalCompoundingSnapshot:
        normalized = {str(k).strip().lower(): max(0.0, float(v)) for k, v in equities.items()}
        total, base = sum(normalized.get(v, 0.0) for v in self.venues), self.current_base(normalized)
        statuses = tuple(VenueCompoundingStatus(
            venue=venue, equity=round(normalized.get(venue, 0.0), 8), target_base=round(base, 8),
        ...
        return GlobalCompoundingSnapshot(
            ...
            global_base=round(base, 8), next_base=round(base * self.growth_factor if all_at_base else base, 8),
