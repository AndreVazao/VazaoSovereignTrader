    ...
    def _validate_order_financial_invariant(self, raw: dict, symbol: str, filled_qty: float, average_price: float) -> dict:
        ...
        if cost is not None:
            ...
            if abs(cost - expected) > tolerance:
                return {"ok": False, "reason": "order_cost_price_quantity_mismatch", "reported_cost": cost, "expected_cost": expected, "tolerance": tolerance}
        fee = raw.get("fee")
        ...
        expected = float(filled_qty) * float(average_price)
        return {"ok": True, "reported_cost": cost, "expected_cost": expected, "relative_tolerance": tolerance_pct}
    ...
    def _reconcile_pending_orders(self) -> None:
        ...
        for order_id, item in list(self.state.pending_orders.items()):
            ...
            try:
                ...
                financial = self._validate_order_financial_invariant(raw, symbol, final_filled, float(raw.get("average") or raw.get("price") or item.get("known_fill_price") or 0.0))
                self.state.financial_reconciliation = {"order_id": order_id, "symbol": symbol, **financial, "checked_at": time.time()}
                if not financial.get("ok", False):
                    self.state.status = "SAFE_MODE"
                    self.log("PENDING_ORDER_FINANCIAL_INVARIANT_BLOCKED", self.state.financial_reconciliation)
                    continue
                cumulative_notional = float(financial.get("reported_cost") if financial.get("reported_cost") is not None else financial.get("expected_cost") or 0.0)
                known_notional = float(item.get("known_quote_notional") or 0.0)
                tolerance = max(1e-12, abs(cumulative_notional) * float(financial.get("relative_tolerance") or 0.002))
                if cumulative_notional + tolerance < known_notional:
                    self.state.status = "SAFE_MODE"
                ...
                if delta > 1e-12:
                    ...
                    if delta_notional <= 0:
                        self.state.status = "SAFE_MODE"
                        self.log("PENDING_ORDER_MISSING_INCREMENTAL_NOTIONAL", {"order_id": order_id, "symbol": symbol, "delta_qty": delta})
                        continue
