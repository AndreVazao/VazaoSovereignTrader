...
class CapitalTransferExecutionBridge:
    ...
    def reconcile(self, *, request: TransferExecutionRequest) -> CapitalTransferState:
        ...
        if self.accounting is not None:
            self.accounting.apply_confirmed(intent_id=request.intent.intent_id, owner_id=self.owner_id,
                source_venue=request.intent.source_venue, destination_venue=request.intent.destination_venue,
                asset=request.intent.asset, amount=request.intent.amount_quote, external_reference=current.external_reference)
        return confirmed

    @staticmethod
    def snapshot(*, enabled: bool) -> dict[str, Any]:
        return {"owner_private": True, "execution_authority": "APPROVED_INTENT_ONLY", "enabled": bool(enabled),
                "confirmation_requires_venue_verification": True, "confirmed_transfer_accounting": True}
