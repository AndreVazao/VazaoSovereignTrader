...
class AdaptiveRiskController:
    ...
    def evaluate(self, *, samples: int, wins: int, mean_net_bps: float, drawdown_pct: float,
                 context_key: str = "global", lower_ci_bps: float = 0.0,
                 evidence_age_ms: int = 0, regime_stability: float = 0.0,
                 independent_samples: int = 0, independent_mean_net_bps: float = 0.0) -> AdaptiveRiskSnapshot:
        ...
        def blocked(reason: str) -> AdaptiveRiskSnapshot:
            return AdaptiveRiskSnapshot(samples, wins, win_rate, float(mean_net_bps), dd, self.min_multiplier, False,
                                        reason, context_key, lower_ci_bps, evidence_age_ms, regime_stability)

        if not self.enabled:
            return AdaptiveRiskSnapshot(samples, wins, win_rate, float(mean_net_bps), dd, self.base_multiplier, False,
                                        "adaptive risk disabled", context_key, lower_ci_bps, evidence_age_ms, regime_stability)
        ...
        return AdaptiveRiskSnapshot(samples, wins, win_rate, float(mean_net_bps), dd, multiplier, True,
